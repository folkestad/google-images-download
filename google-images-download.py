...
def download_concepts_from_file(file_name):
    ...
    with open(file_path) as f:
        ...
        for line in f:
            name = line.strip().replace('\ufeff', '').replace(
                '\xa0', ' ').replace("\'", '').replace('\n', '').split(' ')
            print(counter, [repr(n) for n in name])
            search_keyword = [name[0]]
            if len(name) == 1:
                keywords = ['']
            else:
                keywords = [name[1]]
            start_download_concept(search_keyword, keywords)
            counter += 1
...
############## Main Program ############
def start_download_concept(search_keyword, keywords):
    ...
    while i < len(search_keyword):
        ...
        # IN this saving process we are just skipping the URL if there is any
        # error
...
if __name__ == '__main__':
    ...
    # download_concepts_from_file(file_name='../data/pokemon_151_names.csv')
